      ...
      <div className='bg-slate-100 w-12 h-full rounded-tr-lg rounded-br-lg py-5 text-slate-600 flex flex-col justify-between'>
        ...
        <div className='flex flex-col items-center'>
          ...
          <button title='logout' className='w-12 h-12 flex justify-center items-center cursor-pointer hover:bg-slate-200 rounded' onClick={handleLogout}> 
            <span className=''> <RiLogoutBoxLine size={25}/> </span>
          </button>
        </div>

      </div>
      <div className='w-full'>
        <div className='h-16 flex items-center'><h2 className='text-xl font-bold p-4 text-slate-800'>Message</h2></div>
        ...
        <div className='h-[calc(100vh-65px)] overflow-x-hidden overflow-y-auto scrollbar'>
...

          {
            allUser.map((conv,index)=>{
              return(
                <NavLink to={"/"+conv?.userDetails?._id} key={conv?._id} className='flex items-center gap-2 py-3 px-2 border border-transparent rounded hover:border-primary hover:bg-slate-100 cursor-pointer'>
                  <div>
                    <Avatar
              ...
              {/* unseen messages count */}
                  {
                    Boolean(conv?.unseenMsg) && (<p className='text-xs w-6 h-6 flex justify-center items-center ml-auto p-1 bg-primary font-semibold rounded-full text-white'>{conv?.unseenMsg}</p>)
                  }
                </NavLink>
              )
            })
